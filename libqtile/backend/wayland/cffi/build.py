...
# Create a Makefile include to pass parameters for building wayc as library
with open(QW_PATH / ".." / "cffi" / "libconfig.mk", "w") as f:
    f.write("# Auto-generated by generated by cffi/build.py\n\n")
    f.write(f"QW_PATH := {QW_PATH}\n")
    f.write(f"QW_PROTO_OUT := {QW_PROTO_OUT_PATH}\n")
    f.write(f"CC := {os.getenv(sysconfig.get_config_var('CC'), 'gcc')}\n")
    f.write(f"INCLUDES := {' '.join([f'-I{path}' for path in INCLUDE_DIRS])}\n")
    f.write(f"LDLIBS := {' '.join([f'-l{lib}' for lib in LIBRARIES])}\n")
