...
class _Window:
    ...
    def update_state(self):
        ...
        # This only seems to be an issue with unfullscreening, thus we check if we're fullscreen and the window wants to unfullscreen
            ...
            self._reconfigure_floating(new_float_state=WindowStates.FULLSCREEN)
    ...
    def place(
        ...
        if respect_hints:
            ...
            if "PAspect" in flags and self._float_state == WindowStates.FLOATING:
...
class Window(_Window, base.Window):
    ...
    def fullscreen(self, do_full):
        if do_full:
            needs_change = self._float_state != WindowStates.FULLSCREEN
        ...
        if self._float_state == WindowStates.FULLSCREEN:
    ...
    def minimized(self, do_minimize):
        if do_minimize:
            if self._float_state != WindowStates.MINIMIZED:
                self._enablefloating(new_float_state=WindowStates.MINIMIZED)
        else:
            if self._float_state == WindowStates.MINIMIZED:
                self.floating = False
    ...
    def _reconfigure_floating(self, new_float_state=WindowStates.FLOATING):
        self.update_fullscreen_wm_state(new_float_state == WindowStates.FULLSCREEN)
        ...
        if self._float_state != new_float_state:
            ...
            if new_float_state == WindowStates.FLOATING:
                if self.qtile.config.floats_kept_above:
                    self.keep_above(enable=True)
            elif new_float_state == WindowStates.MAXIMIZED:
                self.move_to_top()
            hook.fire("float_change")

    def _enablefloating(
        self, x=None, y=None, w=None, h=None, new_float_state=WindowStates.FLOATING
    ):
        if new_float_state != WindowStates.MINIMIZED:
            self.x = x
            self.y = y
