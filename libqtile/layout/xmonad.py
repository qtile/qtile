...
class MonadWide(SingleWindow):
    """
    This layout is a rotated version of MonadTall.
    """
    name = "xmonad-wide"
    _top = 0
    _bottom = 1
    ...
    defaults = manager.Defaults(
        ...
        ("border_normal", "#000000", "Border colour for un-focused winows."),
        ("border_width", 2, "Border width."),
        ("name", "xmonad-wide", "Name of this layout."),
    )

    def __init__(self, ratio=_med_ratio, align=_top, change_ratio=.05,
                 change_size=20, **config):
        """
            - ratio       : The percent of the screen-space the
                            master pane should occupy by default.
    ...
    def cmd_normalize(self, redraw=True):
        ...
        if n > 0 and self.group.screen is not None:
            self.sizes = []
            width = self.group.screen.dwidth / n
            # set all sizes to calculated ratio
            for i in range(n):
    ...
    def _maximize_secondary(self):
        ...
        n = len(self.clients) - 2  # total shrinking clients
        # total height of collapsed secondaries
        collapsed_width = self._min_width * n
        nidx = self.focused - 1  # focused size index
        # total height of maximized secondary
        maxed_size = self.group.screen.dwidth - collapsed_width
        # if maximized or nearly maximized
        if abs(self.sizes[nidx] - maxed_size) < self.change_size:
            # minimize
            self._shrink_secondary(self.sizes[nidx] - self._min_width)
        # otherwise maximize
        else:
            self._grow_secondary(maxed_size)
    ...
    def configure(self, c, screen):
        ...
        if self.clients and c in self.clients:
            ...
            # multiple clients
                ...
                # calculate client's x offset
                if self.align == self._top:  # left orientation
                    if self.clients.index(c) == 0:
                        # main client
                ...
                if cidx > 0:
                    # secondary client
                    # ypos is the sum of all clients above it
                    xpos = self.group.screen.dx + sum(self.sizes[:cidx - 1])
                    # get height from precalculated height list
                    try:
                        width = self.sizes[cidx - 2]
                    except:
                        import pdb; pdb.set_trace()
                    # place client based on calculated dimensions
                    c.place(xpos, ypos,
                            width - 2 * self.border_width, height,
                            self.border_width, px)
                    c.unhide()
                else:
                    # main client
                    c.place(self.group.screen.dx, ypos,
                        self.group.screen.dwidth - 2 * self.border_width, height,
                        self.border_width, px)
                    c.unhide()
        else:
            c.hide()
    ...
    def get_shrink_margin(self, cidx):
        "Return how many remaining pixels a client can shrink"
        return max(0, self.sizes[cidx] - self._min_width)
    ...
    def shrink_left_shared(self, cidx, amt):
        """
        Will shrink all secondary clients above the specified
        index by an equal share of the provided amount. After
        applying the shared amount to all affected clients,
        ...
        # apply non-equal shrinkage secondary pass
        # in order to use up any left over shrink amounts
        left = self.shrink_left(cidx, left)
        # return whatever could not be applied
        return left

    def shrink_right(self, cidx, amt):
        """
        Will shrink all secondary clients below the specified
        index in order. Each client will attempt to shrink as
        much as it is able before the next client is resized.
    ...
    def shrink_right_shared(self, cidx, amt):
        """
        Will shrink all secondary clients below the specified
        index by an equal share of the provided amount. After
        applying the shared amount to all affected clients,
        ...
        # apply non-equal shinkage secondary pass
        # in order to use up any left over shrink amounts
        left = self.shrink_right(cidx, left)
        # return whatever could not be applied
        return left
    ...
    def _grow_secondary(self, amt):
        ...
        if self.focused == 1:
            # only shrink downwards
            left -= amt - self.shrink_right_shared(0, amt)
        # last secondary (bottom)
        elif self.focused == len(self.clients) - 1:
            # only shrink upwards
            left -= amt - self.shrink_left(len(self.sizes) - 1, amt)
        # middle secondary
        else:
            # get size index
            idx = self.focused - 1
            # shrink up and down
            left -= half_change_size - self.shrink_left_shared(
                idx, half_change_size)
            left -= half_change_size - self.shrink_right_shared(
                idx, half_change_size)
            left -= half_change_size - self.shrink_left_shared(
                idx, half_change_size)
            left -= half_change_size - self.shrink_right_shared(
    ...
    def _shrink_secondary(self, amt):
        ...
        left = c.width - amt
        # if change would violate min_height
        if left < self._min_width:
            # just reduce to min_height
            change = c.width - self._min_width
        ...
        if self.focused == 1:
            # only grow downwards
            self.grow_right_shared(0, change)
        # last secondary (bottom)
        elif self.focused == len(self.clients) - 1:
            # only grow upwards
            self.grow_left_shared(len(self.sizes) - 1, change)
        # middle secondary
        else:
            idx = self.focused - 1
            # grow up and down
            self.grow_left_shared(idx, half_change)
            self.grow_right_shared(idx, half_change)
        # shrink client by total change
        self.sizes[self.focused - 1] -= change
    ...
    def cmd_flip(self):
        "Flip the layout vertically."
        self.align = self._top if self.align == self._bottom else self._bottom
        self.group.layoutAll()
