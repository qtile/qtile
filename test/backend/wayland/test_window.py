...
@bare_config
def test_window_opacity(manager):
    # 1. Spawn a fake window
    manager.test_window("one")
    # 2. Check the default opacity
    assert manager.c.window.info()["opacity"] == 1.0
    # 3. Change the opacity using the IPC command
    manager.c.window.set_opacity(0.5)
    assert manager.c.window.info()["opacity"] == 0.5
    # 5. Spawn a second window to ensure the compositor doesn't crash
    manager.test_window("two")
    assert manager.c.window.info()["opacity"] == 1.0  # The NEW window is 1.0
    # Test multi window consistency
    manager.c.window.set_opacity(0.2)
    assert window_by_name(manager.c, "two").info()["opacity"] == 0.2  #
    assert window_by_name(manager.c, "one").info()["opacity"] == 0.5


@bare_config
def test_window_opacity_commands(manager):
    manager.test_window("one")
    # 1. Initial state should be 1.0
    assert manager.c.window.info()["opacity"] == 1.0

    # 2. Test explicit out-of-bounds (High)
    manager.c.window.set_opacity(5.0)
    assert manager.c.window.info()["opacity"] == 1.0

    # 3. Test explicit out-of-bounds (Low)
    # The base class IPC command clamps at 0.1, not 0.0!
    manager.c.window.set_opacity(-1.0)
    assert manager.c.window.info()["opacity"] == pytest.approx(0.1)
    ...
    # 5. Test single steps
    manager.c.window.up_opacity()
    assert manager.c.window.info()["opacity"] == pytest.approx(0.6)

    manager.c.window.down_opacity()
    assert manager.c.window.info()["opacity"] == pytest.approx(0.5)
    ...
    assert manager.c.window.info()["opacity"] == pytest.approx(1.0)
